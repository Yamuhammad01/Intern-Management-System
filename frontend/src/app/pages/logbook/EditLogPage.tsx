...
export function EditLogPage({ onNavigate, logId }: EditLogPageProps) {
    ...
    const fetchEntry = async () => {
      try {
        ...
        if (data.success) {
          ...
          setLogDate(new Date(entry.logDate).toISOString().split("T")[0]);
  ...
  const handleSave = async (status: "DRAFT" | "SUBMITTED") => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.message || "Failed to update log entry");
      }
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-800">Edit Log Entry</h1>
            <p className="text-[11px] text-gray-500">Update your internship activity log</p>
          </div>
        </div>
      </div>
